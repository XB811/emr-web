          ...
          <template slot-scope="scope">
            <div class="action-buttons">
              ...
              <emr-change-button
                ...
                :doctor-id="doctorId"
                :department-id="doctorDepartmentId"
                @submit-success="() => handleEmrCreateSuccess(scope.row)"
              >
                <el-button
                  size="mini"
                  type="success"
...
<script>
import { pageQuery, dleteRegistration, finishRegistration } from '@/api/registration'
import { queryByUserId, pageQuery as userPageQuery } from '@/api/user'
...
export default {
  name: 'DoctorRegistrationManagement',
  components: {
    PatientInfoButton,
    EmrChangeButton
  },
  ...
  methods: {
    ...
    fetchDoctorDepartment() {
      if (!this.doctorId) {
        this.$message.error('医生ID不存在，无法获取部门信息')
        return
      }

      queryByUserId(this.doctorId, 'doctor')
        .then(response => {
          if (response && response.data && response.data.departmentId) {
            this.doctorDepartmentId = response.data.departmentId
            console.log('获取到医生部门ID:', this.doctorDepartmentId)
          } else {
            this.$message.warning('未能获取到医生的部门信息')
            console.warn('医生部门信息不存在:', response)
          }
        })
        .catch(error => {
          console.error('获取医生部门信息失败:', error)
          this.$message.error('获取医生部门信息失败')
        })
    },
    ...
    fetchData() {
      ...
      // 添加医生ID和其他参数
      params.doctorId = this.doctorId // 使用当前登录医生的ID

      // 只有在有患者ID时才添加到参数中
      if (this.searchForm.patientIds && this.searchForm.patientIds.length > 0) {
        params.patientIds = this.searchForm.patientIds
      }
    ...
    // 处理病历创建成功后的操作
    handleEmrCreateSuccess(registration) {
      // 病历创建成功后，自动完成挂号
      if (registration && registration.id) {
        finishRegistration(registration.id)
          .then(response => {
            this.$message.success('病历创建成功，挂号已完成')
            this.fetchData() // 刷新数据
          })
          // .catch(error => {
          //   console.error('完成挂号失败:', error)
          //   this.$message.warning('病历创建成功，但挂号完成操作失败')
          //   this.fetchData() // 仍然刷新数据
          // })
      }
    },

    // 原有的完成挂号方法可以保留，但不再直接使用
    handleFinish(row) {
      if (!row || !row.id) {
